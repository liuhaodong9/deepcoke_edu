...
_MODE_INSTRUCTIONS = {
    ...
    "trend": (
        "\n【本次为「趋势模式」】按时间脉络组织回答:根据证据里各文献的年份,"
        "梳理该主题的研究演变(早期→近期分阶段说,标出年份区间),每阶段说明关注点/方法/结论的变化,"
        "用 [N] 标注来源;结尾一句总结趋势方向。证据缺年份时按逻辑演进组织。"
    ),
}
...
def build_answer_prompt(
    ...
    system_prompt = (
        ...
        "  部分 paper 头部有 [实验条件]（煤种/表征方法/性能指标）——陈述因果或结论时，"
        "尽量带上这些条件约束，如 \"在焦煤、经 XRD 表征下，镜质组提高使 CSR 上升 [N]\"，"
        "让结论更严谨、体现实验背景。\n\n"
        "【证据优先级】\n"
